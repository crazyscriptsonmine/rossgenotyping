...
p <- ggplot(data = info, aes(x = No, y = ZHeterozygosity, color=CHROM, label=GENE)) +
	scale_colour_manual(values = vals) + ylim(-7,0) +
  geom_point(stat = "identity", size=0.5) + labs(x = "Chromosome", y=expression(ZH[ W]))
if (min(info\$ZHeterozygosity) <= -3) { p = p + geom_hline(yintercept=c(-2,-3), color="black", linetype="dashed") 
} else { p = p + geom_hline(yintercept=-2, color="black", linetype="dashed") }
p + 
  theme_classic() + 
  ...
  geom_text_repel(aes(label=ifelse(ZHeterozygosity<=-3.4,as.character(GENE),'')),size=3)
dev.off()
RCODE
if ($ARGV[2]){
	$R_code .= <<"RCODE";
